...
// +kubebuilder:validation:XValidation:rule="(has(self.disposable) && self.disposable) || !has(self.storage) || !has(self.storage.data) || (!has(self.storage.data.emptyDir) && (!has(self.storage.data.type) || self.storage.data.type != 'EmptyDir'))",message="data EmptyDir requires disposable=true"
...
// +kubebuilder:validation:XValidation:rule="!has(self.workspaceSharing) || !has(self.workspaceSharing.smb) || !has(self.storage) || !has(self.storage.workspace) || (!has(self.storage.workspace.nfs) && !has(self.storage.workspace.emptyDir) && (!has(self.storage.workspace.type) || self.storage.workspace.type in ['ExistingClaim', 'ClaimTemplate']))",message="SMB workspace sharing requires a claim-backed workspace"
